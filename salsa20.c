...
{
	s20_quarterround(&y[0], &y[1], &y[2], &y[3]);			// 0000 , 0001,  0010, 0011
	s20_quarterround(&y[5], &y[6], &y[7], &y[4]);			// 0101 , 0110,  0111, 0100
	s20_quarterround(&y[10], &y[11], &y[8], &y[9]);		// 1010 , 1011,  1000, 1001
	s20_quarterround(&y[15], &y[12], &y[13], &y[14]);		// 1111,  1100,  1101, 1110
	// Binär geordnet !	
}

static void s20_columnround(uint32_t x[static 16])
{
	s20_quarterround(&x[0], &x[4], &x[8], &x[12]);			// 0000, 0100, 1000, 1100  //erste 2 bitziffern ändern sich + 00
	s20_quarterround(&x[5], &x[9], &x[13], &x[1]);			// 0101, 1001, 1101, 0001  //erste 2 bitziffern ändern sich + 01
	s20_quarterround(&x[10], &x[14], &x[2], &x[6]);		// 1010, 1110, 0010, 0110 //erste 2 bitziffern ändern sich + 10
	s20_quarterround(&x[15], &x[3], &x[7], &x[11]);		// 1111, 0011, 0111, 1011  //erste 2 bitziffern ändern sich + 11
}
						  ...
						  uint8_t keystream[static 64])
{
	...
	// Copy all 'tau' into correct spots in our keystream block
	for (i = 0; i < 64; i+=20)
		for (j = 0; j < 4; ++j)
						  ...
						  uint8_t keystream[static 64])
{
	...
	// Copy all 'sig' into correct spots in our keystream block
	for (i = 0; i < 64; i+=20)
		for (j = 0; j < 4; ++j)
...
// Performs up to 2^32 -1 bytes of encryption/decryption under a 
							...
							uint32_t buflen)
{
	...
	// with the per-block 'counter' value (4 bytes in our case, 8 bytes standard).
	// We leave the high 4 bytes set to zero because we permit only a 32- bit int
	// from stram idx and length
	uint32_t n[16] = { 0 };
	uint32_t i;
	...
	// If we're not on block boundr, compute the first keystream block
	// This will make the primary loop below cleaner
	if (si % 64 != 0)
	{
	...
	for (i = 0; i < buflen; ++i) 
	{
		...
		//xor the byte of plaintext with one byte of keystream
